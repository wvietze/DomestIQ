-- Update search_workers to also match via worker_service_areas and return location_name
-- Must drop first because return type is changing (adding location_name)
DROP FUNCTION IF EXISTS search_workers(DOUBLE PRECISION, DOUBLE PRECISION, INTEGER, UUID, NUMERIC, INTEGER, INTEGER, INTEGER);
...
BEGIN
  ...
  WHERE wp.is_active = true
    ...
    AND (
      ...
      -- Match on primary location
      (wp.location IS NOT NULL AND ST_DWithin(wp.location, ST_SetSRID(ST_MakePoint(p_lng, p_lat), 4326)::geography, p_radius_km * 1000)) OR
      -- Match on any service area
      wp.id IN (
        SELECT wsa.worker_id FROM worker_service_areas wsa
        WHERE ST_DWithin(wsa.center, ST_SetSRID(ST_MakePoint(p_lng, p_lat), 4326)::geography, wsa.radius_km * 1000)
      )
    )
  ORDER BY
