...
export function WorkerRegisterWizard() {
 ...
 {STEPS.map((s, i) => (
 <div key={s.id} className={cn('h-2.5 rounded-full transition-all duration-300', i === step ? 'w-8 bg-primary' : i < step ? 'w-2.5 bg-primary/60' : 'w-2.5 bg-border')} />
 ))}
 </div>
 ...
 className={cn('flex items-center gap-3 p-4 rounded-xl border-2 transition-all text-left', isSelected ? 'border-primary bg-primary/5 shadow-sm' : 'border-border hover:border-emerald-300')}>
 ...
 className={cn('flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all min-h-[72px]', formData.availableDays.includes(day.idx) ? 'border-primary bg-primary text-primary-foreground shadow-sm' : 'border-border hover:border-emerald-300')}>
 ...
 <label className="flex items-start gap-3 p-4 rounded-xl border cursor-pointer hover:bg-muted transition-colors">
