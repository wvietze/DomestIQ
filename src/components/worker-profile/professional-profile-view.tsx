...
export function ProfessionalProfileView({ data }: { data: WorkerProfileViewData }) {
 ...
 {svc.skill_level && (
 <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${SKILL_COLORS[svc.skill_level] || 'bg-muted text-muted-foreground'}`}>
 {svc.skill_level}
 </span>
 )}
 ...
 <div key={review.id} className="p-3 rounded-xl bg-muted space-y-1">
 <div className="flex items-center gap-1">
 {[1, 2, 3, 4, 5].map(s => (
 ...
 <div className="space-y-3">
 {references.map(ref => (
 <div key={ref.id} className="p-3 rounded-xl bg-muted space-y-1">
