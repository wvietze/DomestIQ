...
export function SimpleProfileView({ data }: { data: WorkerProfileViewData }) {
 ...
 <circle cx="74"cy="74"r="62"fill="none"className="stroke-gray-200"strokeWidth="4"/>
 <circle
 cx="74"cy="74"r="62"
 fill="none"
 ...
 >
 <div className="w-16 h-16 rounded-2xl bg-card border border-emerald-200 flex items-center justify-center shadow-sm">
 ...
 profile.id_verified ? 'bg-emerald-100' : 'bg-muted'
 }`}>
 {profile.id_verified
 ...
 <div className={`w-14 h-14 rounded-full flex items-center justify-center ${
 profile.criminal_check_clear ? 'bg-emerald-100' : 'bg-muted'
 }`}>
 {profile.criminal_check_clear
 ? <ShieldCheck className="w-7 h-7 text-emerald-600"/>
 : <XCircle className="w-7 h-7 text-muted-foreground"/>
