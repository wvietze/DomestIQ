...
import { createReferenceSchema, parseBody } from '@/lib/validations/api'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = parseBody(createReferenceSchema, body)
    if (!parsed.success) {
      return NextResponse.json({ error: parsed.error }, { status: 400 })
    }
    const { worker_id, reference_text, relationship, duration_months, booking_id } = parsed.data
